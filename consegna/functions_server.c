...
int delay_decypher = 0; // 40 per test
int delay_writing = 0;  // 40 per test
...
void termination_handler(int signum)
{
    ...
    {
        printf("[SERVER] Termination signal received. Closing server...\n\n");
    ...
    {
        pid_t tid = (pid_t)syscall(SYS_gettid);
        printf("[SERVER] Termination signal received in thread %d. Exiting thread...\n\n", tid);
        sem_post(&available_connections);
    }
    pthread_exit(NULL);
}
